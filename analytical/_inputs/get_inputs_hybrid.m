%% GET SPENDER-OLG HYBRID AGGREGATE CONSUMPTION FUNCTION
...
C_i = mu * zeros(T) + (1-mu) * C_i;
D_i = mu * zeros(T) + (1-mu) * D_i;
...
C_pi = mu * zeros(T) + (1-mu) * C_pi;
D_pi = mu * zeros(T) + (1-mu) * D_pi;

%----------------------------------------------------------------
% Interest Rate (no income effect)
%----------------------------------------------------------------
...
for t = 1:T
    exo.zeta_hat(t) = step;
    [c_shock,d_shock] = c_hybrid_fn(exo,A_inv);
    C_zeta(:,t) = (c_shock-c_base)/step;
    D_zeta(:,t) = (d_shock-d_base)/step;
    exo.zeta_hat(t) = 0;
end

C_zeta = mu * zeros(T) + (1-mu) * C_zeta;
D_zeta = mu * zeros(T) + (1-mu) * D_zeta;
...
C_d = mu * zeros(T) + (1-mu) * C_d;
D_d = mu * zeros(T) + (1-mu) * D_d;

%% SAVE RESULTS

cd([path experiment '/_results']);

save inputs_hybrid C_y C_i C_pi C_d D_y D_i D_pi D_d ...
    beta omega sigma ...
