function [c_opt,ap_opt,mutilde_upd] = EGP_fun(mutilde_opt,r_b_grid,Y,Trans,b_shock,...
...
c_endo = repmat((beta./exp(b_shock) .* mutilde_opt).^(-1/gamma),1,n_yT);
...
mutilde_upd = Emat_yP * (((1 + r_b_grid) .* exp(b_shock) .* c_opt.^(-gamma)) * yT_dist');
