...
ib_seq(end)    = vars(2);
pi_seq(end)    = vars(3);
tau_x_seq(end) = vars(4);
tau_e_seq(end) = vars(5);
eps_b_seq(end) = vars(6);
...
Trans_seq   = NaN(n_s,T);
for t = 1:T
    Trans_seq(:,t) = Trans_SS * exp(tau_x_seq(t) + tau_e_seq(t));
end
...
for t = T:-1:1
    [c_opt_t(:,:,t),ap_opt_t(:,:,t),mutilde_t(:,t)] = EGP_fun(mutilde_t(:,t+1),r_b_grid_seq(:,t),Y_seq(t),Trans_seq(:,t),eps_b_seq(t),...
